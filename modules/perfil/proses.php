...
if (empty($_SESSION['username']) && empty($_SESSION['password'])) {
    echo "<meta http-equiv='refresh' content='0; irl=index.php?alert=3'>";
} else {
    if ($_GET['act'] == "update") {
        if (isset($_POST['Guardar'])) {
            if (isset($_POST['id_user'])) {
                $id_user = mysqli_real_escape_string($mysql, trim($_POST['id_user']));
                $username = mysqli_real_escape_string($mysql, trim($_POST['username']));
                $name_user = mysqli_real_escape_string($mysql, trim($_POST['name_user']));
                $email = mysqli_real_escape_string($mysql, trim($_POST['email']));
                $telefono = mysqli_real_escape_string($mysql, trim($_POST['telefono']));
                ...
                //Solucionar error de guardar que explota si no cargas la imagen antes
                if (empty($_FILES['foto']['name'])) {
                    $query = mysqli_query($mysql, "UPDATE usuarios SET username = '$username',
                                                                                     name_user = '$name_user',
                                                                                     email = '$email',
                                                                                     telefono = '$telefono'
